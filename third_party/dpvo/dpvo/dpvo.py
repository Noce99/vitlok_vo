...
from .utils import *
from .nut_depth import save_depth_visualization
...
class DPVO:
    ...
    def __init__(self, cfg, network, ht=480, wd=640, viz=False):
        ...
        self.M = self.cfg.PATCHES_PER_FRAME # 96
        self.N = self.cfg.BUFFER_SIZE       # 4096
        ...
        self.image_ = torch.zeros(self.ht, self.wd, 3, dtype=torch.uint8, device="cpu") # H, W, 3
        ...
        self.imap_ = torch.zeros(self.pmem, self.M, DIM, **kwargs)                  # 36, 96, 384
        self.gmap_ = torch.zeros(self.pmem, self.M, 128, self.P, self.P, **kwargs)  # 36, 96, 128, 3, 3
        ...
        self.fmap1_ = torch.zeros(1, self.mem, 128, ht // 1, wd // 1, **kwargs)     # 1, 36, 128, H / 8, W / 8
        self.fmap2_ = torch.zeros(1, self.mem, 128, ht // 4, wd // 4, **kwargs)     # 1, 36, 128, H / 32, W / 32
    ...
    def load_weights(self, network):
        ...
        self.DIM = self.network.DIM # 384
        self.RES = self.network.RES # 4
        self.P = self.network.P     # 3
    ...
    def __call__(self, tstamp, image, intrinsics, depth=None):
        ...
        image = 2 * (image[None,None] / 255.0) - 0.5   # The image pass from values in [0;255] to values in [-0.5;1.5]
        
        # image 1, 1, 3, 528, 960
        # ~ channels, H / 2, W / 2 just ~ because they need to be integer divisor of 16
        ...
        with autocast(enabled=self.cfg.MIXED_PRECISION):
            fmap, gmap, imap, patches, _, clr = \
                self.network.patchify(image,
                    ...
                    centroid_sel_random_ratio=self.cfg.CENTROID_SEL_RANDOM_RATIO,
                    depth_map=depth,
                    return_color=True)

        
        # fmap    1, 1, 128, 132, 240
        # gmap    1, 96, 128, 3, 3
        # imap    1, 96, 384, 1, 1
        # patches 1, 96, 384, 1, 1    [0.0; 255.0]
        # clr     1, 96, 3            [-0.5; 1.5]
        ...
        self.pg.index_[self.n + 1] = self.n + 1 # It sets all the 96 values of self.pg.index_[self.n + 1] to self.n + 1

        self.pg.index_map_[self.n + 1] = self.m + self.M # self.M is 96
        ...
        if depth is not None:
            # Sample metric depth at patch centers (feature-map coords × RES → image coords)
            u = (patches[0, :, 0, 1, 1] * self.RES).long().clamp(0, self.wd - 1)
            v = (patches[0, :, 1, 1, 1] * self.RES).long().clamp(0, self.ht - 1)
            d = depth.squeeze().cuda()
            d_vals = d[v, u]
            valid = d_vals > 0
            if valid.any():
                inv_d = torch.where(valid, 1.0 / d_vals.clamp(min=1e-3), d_vals.new_zeros(1))
                inv_d[~valid] = inv_d[valid].median()
                patches[:, :, 2] = inv_d[None, :, None, None]
            else:
        ...
        #     print(patches.shape)
        #     save_depth_visualization(image, self.pg.patches_[None, self.n-1], self.ht, self.wd, frame_idx=self.n)
