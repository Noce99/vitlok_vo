...
class PatchGraph:
    ...
    def __init__(self, cfg, P, DIM, pmem, **kwargs):
        ...
        self.tstamps_ = np.zeros(self.N, dtype=np.int64)                                                 # 4096
        self.poses_ = torch.zeros(self.N, 7, dtype=torch.float, device="cuda")                           # 4096, 7
        ...
        self.index_ = torch.zeros(self.N, self.M, dtype=torch.long, device="cuda")                       # 4096, 96
        self.index_map_ = torch.zeros(self.N, dtype=torch.long, device="cuda")                           # 4096
        ...
        ### edge information ###
        self.net = torch.zeros(1, 0, DIM, **kwargs)                                                       # 1, 0, 384
        ...
        self.weight_inac = torch.zeros(1, 0, 2, dtype=torch.float, device="cuda")
        self.target_inac = torch.zeros(1, 0, 2, dtype=torch.float, device="cuda")
