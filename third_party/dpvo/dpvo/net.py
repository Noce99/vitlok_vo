...
class Patchifier(nn.Module):
    ...
    def forward(self, images, patches_per_image=80, disps=None, centroid_sel_strat='RANDOM', centroid_sel_random_ratio=0.5, depth_map=None, return_color=False):
        ...
        elif centroid_sel_strat == 'DEPTH_HALF_RANDOM':
            half = round(patches_per_image * centroid_sel_random_ratio)
            near = patches_per_image - half

            x_rand = torch.randint(1, w-1, size=[n, half], device="cuda")
            y_rand = torch.randint(1, h-1, size=[n, half], device="cuda")

            if depth_map is not None:
                d = torch.as_tensor(depth_map, dtype=torch.float32).cuda().squeeze()
                d_small = F.interpolate(d[None, None], size=(h, w), mode='bilinear', align_corners=False).squeeze()

                x_depth_list, y_depth_list = [], []
                for i in range(n):
                    xc = torch.randint(1, w-1, size=[near * 20], device="cuda")
                    yc = torch.randint(1, h-1, size=[near * 20], device="cuda")
                    d_vals = d_small[yc, xc]
                    mask = (d_vals > 0) & (d_vals < 10.0)
                    xi, yi = xc[mask], yc[mask]
                    if xi.shape[0] >= near:
                        idx = torch.randperm(xi.shape[0], device="cuda")[:near]
                        xi, yi = xi[idx], yi[idx]
                    else:
                        xf = torch.randint(1, w-1, size=[near - xi.shape[0]], device="cuda")
                        yf = torch.randint(1, h-1, size=[near - xi.shape[0]], device="cuda")
                        xi, yi = torch.cat([xi, xf]), torch.cat([yi, yf])
                    x_depth_list.append(xi)
                    y_depth_list.append(yi)

                x_depth = torch.stack(x_depth_list)
                y_depth = torch.stack(y_depth_list)
            else:
                x_depth = torch.randint(1, w-1, size=[n, near], device="cuda")
                y_depth = torch.randint(1, h-1, size=[n, near], device="cuda")

            x = torch.cat([x_rand, x_depth], dim=1)
            y = torch.cat([y_rand, y_depth], dim=1)

        elif centroid_sel_strat == 'DEPTH_CLOSE':
            if depth_map is not None:
                d = torch.as_tensor(depth_map, dtype=torch.float32).cuda().squeeze()
                d_small = F.interpolate(d[None, None], size=(h, w), mode='bilinear', align_corners=False).squeeze()
                ...
                x_list, y_list = [], []
                for i in range(n):
                    xc = torch.randint(1, w-1, size=[patches_per_image * 20], device="cuda")
                    yc = torch.randint(1, h-1, size=[patches_per_image * 20], device="cuda")
                    d_vals = d_small[yc, xc]
                    mask = (d_vals > 0) & (d_vals < 10.0)
                    xi, yi = xc[mask], yc[mask]
                    if xi.shape[0] >= patches_per_image:
                        idx = torch.randperm(xi.shape[0], device="cuda")[:patches_per_image]
                        xi, yi = xi[idx], yi[idx]
                    else:
                        xf = torch.randint(1, w-1, size=[patches_per_image - xi.shape[0]], device="cuda")
                        yf = torch.randint(1, h-1, size=[patches_per_image - xi.shape[0]], device="cuda")
                        xi, yi = torch.cat([xi, xf]), torch.cat([yi, yf])
                    x_list.append(xi)
                    y_list.append(yi)
